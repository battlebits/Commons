package br.com.battlebits.commons.translate;

public class TranslateTag {

    /**
     * Account
     */
    public final static String ACCOUNT_NOT_LOADED = "account.not-loaded";
    public final static String ACCOUNT_ALREADY_ONLINE = "account.already-online";
    public final static String ACCOUNT_LOAD_FAILED = "account.load-failed";

    /**
     * Command
     */
    public final static String COMMAND_NOT_FOUND = "command.not-found";
    public final static String COMMAND_NO_PERMISSION = "command.no-permission";
    public final static String CANT_TYPE_TWO_DOT_COMMAND = "command.cant-type-two-dot";

    /**
     * Server
     */
    public final static String SERVER_IS_ON_WHITELIST = "server.is-on-whitelist";
}
